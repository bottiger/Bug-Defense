...
public class TowerDefenseView_snake extends TileView {
	 ...
	 */
	private ArrayList<Coordinate> mTDTrail = new ArrayList<Coordinate>();
	private ArrayList<Coordinate> mAppleList = new ArrayList<Coordinate>();
	private Route r = null;

	/**
	 * creepList: a list of Creeps which is currently moving around
	 * towerList: the location of the defense towers.
	 */
	private ArrayList<Creep> creepList = new ArrayList<Creep>();
	...
	/**
	 * Constructs a TDView based on inflation from XML
	 * 
	 * @param context
	 * @param attrs
	 */
	public TowerDefenseView_snake(Context context, AttributeSet attrs) {
		super(context, attrs);
		initTDView();
		//startGame();
	}

	public TowerDefenseView_snake(Context context, AttributeSet attrs, int defStyle) {
	...
	private void initNewGame() {
		...
		checkPoints.add(new Coordinate(7, 33));
		checkPoints.add(new Coordinate(18, 22));
		
		Coordinate creepStart = new Coordinate(0,0);
		Coordinate creepEnd = new Coordinate(24,30);
		...
		mTDTrail.add(new Coordinate(7, 7));
		mTDTrail.add(new Coordinate(6, 7));
		mTDTrail.add(new Coordinate(5, 7));
		mTDTrail.add(new Coordinate(4, 7));
		mTDTrail.add(new Coordinate(3, 7));
		mTDTrail.add(new Coordinate(2, 7));
	...
	public Bundle saveState() {
		...
		map.putIntArray("mSnakeTrail", coordArrayListToArray(mTDTrail));
	...
	public void restoreState(Bundle icicle) {
		...
		mTDTrail = coordArrayToArrayList(icicle.getIntArray("mSnakeTrail"));
	}
	...
	private void addRandomApple() {
		...
		while (!found) {
			...
			int snakelength = mTDTrail.size();
			for (int index = 0; index < snakelength; index++) {
				if (mTDTrail.get(index).equals(newCoord)) {
					collision = true;
				}
			}
	...
	private void drawRoute(Route route) {
		...
		for (Coordinate c : route.getPath())
		{
	...
	private void updateSnake() {
		...
		Coordinate head = mTDTrail.get(0);
		...
		for (int snakeindex = 0; snakeindex < snakelength; snakeindex++) {
			Coordinate c = mTDTrail.get(snakeindex);
			if (c.equals(newHead)) {
				setMode(LOSE);
				return;
			}
		...
		mTDTrail.add(0, newHead);
		// except if we want the snake to grow
		if (!growSnake) {
			mTDTrail.remove(mTDTrail.size() - 1);
		}

		int index = 0;
		for (Coordinate c : mTDTrail) {
			if (index == 0) {
				setTile(YELLOW_STAR, c.x, c.y);
			} else {
				setTile(RED_STAR, c.x, c.y);
